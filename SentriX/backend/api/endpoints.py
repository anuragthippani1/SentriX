...
async def list_sessions(chatbot: ChatbotManager = Depends(get_chatbot_manager)):
    """List all active chat sessions."""
    sessions = chatbot.list_sessions()  # You’ll need to implement this in ChatbotManager if not already
    return sessions

@router.get("/sessions/{session_id}", response_model=Session)
async def get_session(session_id: str, chatbot: ChatbotManager = Depends(get_chatbot_manager)):
    """Fetch a specific chat session by ID."""
    session = chatbot.get_session(session_id)  # Also needs to exist in ChatbotManager
    if not session:
